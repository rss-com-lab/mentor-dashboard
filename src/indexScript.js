...
const getScore = (sheet, currentRow) => {
  ...
  if (sheet[fieldMappingScore.action + currentRow]
  ...
  while (sheet[fieldMappingScore.mentorGithub + count]) {
    ...
    if (dismissedStudents.indexOf(studentsList.get(student.studentName).studentName) !== -1) {
      ...
      if (sheet[fieldMappingScore.reasonDismiss + count]) {
        studentsList.get(student.studentName)
          .reasonDismiss = sheet[fieldMappingScore.reasonDismiss + count].v;
      }
...
const getCountTask = (sheet, nameTask) => {
  ...
  let countTask = 0;
  while (sheet[fieldMappingScore.mentorGithub + count]) {
    if (sheet[fieldMappingScore.task + count]
