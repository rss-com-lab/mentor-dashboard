...
// eslint-disable-next-line
const studentList = XLSX.readFile(
  './src/data/Score RSSchool 2018Q3 (Responses) 2019-03-11.xlsx',
);
...
const getScore = (sheet, currentRow) => {
  const studentData = {
    ...
    studentName: sheet[fieldMappingScore.studentGithub + currentRow].v.split(
      loginPattern,
    )[1]
      ? sheet[fieldMappingScore.studentGithub + currentRow].v
        .split(loginPattern)[1]
        .toLowerCase()
      : sheet[fieldMappingScore.studentGithub + currentRow].v.split(
        loginPattern,
      )[0],
  ...
  if (
    sheet[fieldMappingScore.action + currentRow]
    && sheet[fieldMappingScore.action + currentRow].v === 'Отчислить студента'
  ) {
...
const getTasks = (sheet) => {
  ...
  while (sheet[fieldMappingScore.mentorGithub + count]) {
    ...
    if (
      dismissedStudents.indexOf(
        studentsList.get(student.studentName).studentName,
      ) !== -1
    ) {
      studentsList.get(student.studentName).studentStatus = 'dismissed';
      if (sheet[fieldMappingScore.reasonDismiss + count]) {
        studentsList.get(student.studentName).reasonDismiss
          = sheet[fieldMappingScore.reasonDismiss + count].v;
      }
    } else {
      studentsList.get(student.studentName).studentStatus = 'active';
    }
...
const getCountTask = (sheet, nameTask) => {
  ...
  while (sheet[fieldMappingScore.mentorGithub + count]) {
    if (
      sheet[fieldMappingScore.task + count]
      && sheet[fieldMappingScore.task + count].v === nameTask
    ) {
      countTask += 1;
    }
...
const getStatusTask = (sheet, currentRow, sheetStudent) => ({
  ...
  checkTaskTime: sheet[fieldMappingStatus.checkTaskTime + currentRow]
    ? sheet[fieldMappingStatus.checkTaskTime + currentRow].v
    : '',
});
